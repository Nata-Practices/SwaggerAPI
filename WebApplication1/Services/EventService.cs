...
{
    Task<List<Model.Event>> GetAllEventsAsync();
    Task<Model.Event> GetEventByIdAsync(string id);
    Task<Model.Event> AddEventAsync(Model.Event newEvent);
    Task<Model.Event> UpdateEventAsync(string id, Model.Event updatedEvent);
    Task<Model.Event> DeleteEventAsync(string id);
}

public class EventService : IEventService
{
    private readonly IMongoCollection<Model.Event> _events;
    ...
    public EventService(IMongoClient client, IOptions<MongoDBSettings> settings, IDistributedCache cache, ILogger<EventService> logger)
    {
        ...
        _events = database.GetCollection<Model.Event>("Events");
        _cache = cache;
    ...
    public async Task<List<Model.Event>> GetAllEventsAsync()
    {
        ...
        if (!string.IsNullOrEmpty(cachedData))
        {
            ...
            return JsonSerializer.Deserialize<List<Model.Event>>(cachedData);
        }
    ...
    public async Task<Model.Event> GetEventByIdAsync(string id)
    {
        ...
        if (!string.IsNullOrEmpty(cachedData))
        {
            ...
            return JsonSerializer.Deserialize<Model.Event>(cachedData);
        }
    ...
    public async Task<Model.Event> AddEventAsync(Model.Event newEvent)
    {
    ...
    public async Task<Model.Event> UpdateEventAsync(string id, Model.Event updatedEvent)
    {
        updatedEvent.Id = id;
    ...
    public async Task<Model.Event> DeleteEventAsync(string id)
    {
